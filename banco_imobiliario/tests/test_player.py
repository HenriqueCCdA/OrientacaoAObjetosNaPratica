...
from monopoly.player import (AbortInvestment, Cautions, Demanding, Gambler, Player,
                            OutOfMoney,
                            NotEnoughMoney,
                            Impulsive)
...
def test_invest_without_money():
    ...
    with pytest.raises(NotEnoughMoney):
        assert p.invest(price=100, rent=10)
...
def test_invest_cautions():
    p = Player(180, strategy=Cautions())
    assert p.invest(price=100, rent=50) == 100
    assert p.balance == 80

def test_invest_cautions_abort():
    p = Player(100, strategy=Cautions())
    with pytest.raises(AbortInvestment):
        p.invest(price=100, rent=10)

def test_invest_gambler(mocker):
    p = Player(180, strategy=Gambler())
...
def test_factory():
    ...
    assert names == ['Impulsive', 'Demanding', 'Cautions', 'Gambler']
